...
**Kết quả kiểm thử:** `tsc`/`lint`/`npm run build` sạch sau từng thay đổi; `tests/integration/mcp-server.test.ts` 30/30 pass; test cho 4 service vừa sửa/thêm (`ads`/`admin-expense`/`admin-receipt`/`page-service`/`salary-history-flow`, 45/45) pass; full `npm run test` (175/175, 23 file) chạy lại sau cùng để xác nhận không có regression ở các suite khác.
