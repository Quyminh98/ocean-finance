...
**Cập nhật ngày 2026-08-22 (rà soát đồng bộ MCP tool ↔ tính năng thật lần 2, theo yêu cầu user):** dòng tổng bên dưới trước đó (40 tool) đã lệch code thật — 4 tool `list_sellers`/`create_seller`/`update_seller`/`delete_seller` (Seller, mục ~15.2a) được thêm ngày 2026-08-20 nhưng chưa từng cập nhật lại tổng ở mục này, chỉ ghi trong `schema.md`. Rà soát toàn bộ Service Layer (`server/services/*.ts`) so với `registerTool()` phát hiện thêm các gap thật (entity có sẵn CRUD ở Web nhưng MCP chưa có tool, không phải quyết định có chủ đích như `EmployeeProfitSettlement`):
- **Bug đã fix:** `update_page` xoá mất `payoutId` của Page ở MỌI lần gọi — vì `updatePage()` ghi `payoutId: input.payoutId ?? null` không điều kiện (full-replace semantics, giống `notes`) nhưng input schema của tool chưa từng nhận field này. Đã thêm `payoutId` vào input `create_page` lẫn `update_page`.
- **Payout** (mục ~15.2b) — thêm `list_payouts`, `create_payout`, `update_payout`, `delete_payout` (hard delete, cùng lý do Seller — metadata thuần tuý).
- **Via** (mục ~20b) — thêm `list_vias`, `create_via`, `update_via`, `delete_via`. Khác Web (chỉ tự tạo/sửa/xoá Via của chính mình): MCP nhận `holderUserId` tường minh và dùng quyền Admin Full để quản lý Via của bất kỳ ai (quyết định user, không phải suy diễn) — `update_via`/`delete_via` vẫn reject (`FORBIDDEN`) nếu `holderUserId` truyền vào không khớp đúng chủ sở hữu hiện tại của Via, `via.service.ts` không đổi gì (MCP truyền thẳng `holderUserId` vào đúng vị trí "acting user" mà service đã kiểm tra sẵn).
- **SalaryHistory** — thêm `get_salary_history` (đọc lịch sử đổi lương đầy đủ; `set_employee_salary`/`get_employee_detail` trước giờ chỉ ghi/đọc mức hiện tại).
- **PageAssignment** — thêm `get_page_assignment_history`, `get_employee_assignment_history` (đọc lịch sử phụ trách/chuyển giao — trước giờ chỉ có tool ghi `assign_employee`/`transfer_page`).
- **PagePurchaseExpense** — thêm `list_page_purchase_expenses_by_employee`, `list_page_purchase_expenses_by_admin`.
- **AdminExpense** — thêm `restore_admin_expense` (hoàn tác soft-delete, song song `softDeleteAdminExpense` đã có tool từ Phase 16).
- **Dashboard** — thêm `get_dashboard_monthly_series`, `get_admin_spending_breakdown`, `get_recent_activity` (trước giờ `get_dashboard` chỉ gọi `getSystemFinancials`, bỏ sót 3 hàm còn lại của `dashboard.service.ts`).

Vẫn cố tình chưa có MCP tool (không phải gap): `EmployeeProfitSettlement` (quyết định có chủ đích, xem `plan.md`), `McpClient` tự quản lý API key của chính nó (rủi ro bảo mật nếu để AI agent tự tạo/thu hồi key).

**Tổng hiện tại: 61 tool** (24 read + 37 write, write gồm cả destructive) — đã đối chiếu trực tiếp với `registerTool()` trong `src/mcp/server.ts` (2026-08-22) để đảm bảo con số này không lệch code thật. Test tích hợp cũ trong `tests/integration/mcp-server.test.ts` (round-trip create/update/delete + list cho `EmployeeReceipt`/`PageStatusOption`, `delete_page_status_option` có assertion reject khi thiếu `confirm`, `list_admins` tìm đúng fixture Admin) — 30/30 test MCP pass tính đến 2026-08-20; **các tool mới thêm ngày 2026-08-22 (Payout/Via/SalaryHistory-read/PageAssignment-read/PagePurchaseExpense-read/restore_admin_expense/Dashboard-read) chưa có test tích hợp riêng** — đã qua `tsc --noEmit` + `next build` sạch, nhưng chưa chạy `vitest` (bộ test tích hợp ghi thẳng vào DB thật, không có DB test riêng ở môi trường này).
