...
export default async function AdminDetailPage({ params }: { params: Promise<{ adminId: string }> }) {
  ...
  const [adExpenses, pagePurchaseExpenses, activeSalaries, adminExpenses, adminReceipts] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="mb-gutter">
        <h2 className="mb-stack-sm font-headline-sm text-headline-sm text-on-surface">Chi tiết đã chi</h2>
        {costRows.length === 0 ? (
      ...
      <div>
        <h2 className="mb-stack-sm font-headline-sm text-headline-sm text-on-surface">Chi tiết đã nhận</h2>
