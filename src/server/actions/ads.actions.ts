...
export async function createAdExpenseAction(input: unknown): Promise<CreateAdExpenseState> {
  ...
  try {
    ...
    if (error instanceof AdExpenseError || error instanceof PageError) return { status: "error", error: error.message };
    throw error;
  }
}
...
export async function deleteAdExpenseAction(adExpenseId: string, pageId: string): Promise<DeleteAdExpenseState> {
  const admin = await requireAdmin();
